usage_genesis() {
    ...
    prepare PROFILE-JSON TOPO-DIR OUTDIR
...
case "${op}" in
    ...
    prepare )
        local usage="USAGE:  wb genesis prepare-cache-entry PROFILE-JSON TOPO-DIR OUTDIR"
        ...
        echo "Preparing genesis cache entry $cache_key:  $cache_hit_desc"
        ...
        then echo "Generating genesis due to ${regenesis_causes[*]}:  $cache_key @$cache_path"
    ...
    actually-genesis )
        ...
        cli genesis create --genesis-dir "$dir"/ \
            $(jq '.cli_args.createSpec' "$profile_json")
        ...
        params=(--genesis-dir "$dir"
                $(jq '.cli_args.createFinalBulk' "$profile_json")
               )
    ...
    finalise-cache-entry )
        ...
        ## Decide start time:
        local system_start_epoch=$(date '+%s' --date="now + $(jq .genesis.genesis_future_offset "$profile_json")")
...
Massage_the_key_file_layout_to_match_AWS() {
    ...
    echo "genesis: pool density map:  $pool_density_map"
